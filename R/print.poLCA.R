#' @export
print.poLCA <- function(x, ...) {
  R <- length(x$P)
  S <- ifelse(is.na(x$coeff[1]), 1, nrow(x$coeff))
  cat(paste("Conditional item response (column) probabilities,
  ...
  if (x$numiter == x$maxiter) {
    cat("ALERT: iterations finished, MAXIMUM LIKELIHOOD NOT FOUND \n \n")
  }
  if (!x$probs.start.ok) {
    cat(paste(
      "ALERT: error in user-specified starting values;",
      "new start values generated \n \n"
    ))
  }
  if (x$npar > x$N) {
    cat(
      "ALERT: number of parameters estimated (",
      x$npar, ") exceeds number of observations (", x$N, ") \n \n"
    )
  }
  if (x$resid.df < 0) {
    cat("ALERT: negative degrees of freedom; respecify model \n \n")
  }
  if (x$eflag) {
    cat(paste(
      "ALERT: estimation algorithm automatically restarted with",
      "new initial values \n \n"
    ))
  }
  utils::flush.console()
